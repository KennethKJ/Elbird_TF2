    ...
    if counter >= frames_btw_obj_detect and doNN:
        ...
        if X is not None and bounding_boxes != []:
            ...
            for i, bb in enumerate(bounding_boxes):
                ...
                x1 = mid_x - int(win_size[1]/2)
                x2 = x1 + win_size[1]  # mid_x + int(target_img_size[1]/2)

                if x1 < 0:
                    x2 = x2 + abs(x1)
                    x1 = 0

                if x2 > raw_image_shape[1]:
                    x2 = raw_image_shape[1]
                    x1 = x1 - (x2 - raw_image_shape[1])


                y1 = mid_y - int(win_size[0]/2)
                y2 = y1 + win_size[0]  #  + int(target_img_size[0]/2)

                if y1 < 0:
                    y2 = y2 + abs(y1)
                    y1 = 0

                if y2 > raw_image_shape[0]:
                    y2 = raw_image_shape[0]
                    y1 = y1 - (y2 - raw_image_shape[0])

                NN_boxes.append((x1, x2, y1, y2))

                img_snippet = frame[y1: y2, x1: x2, :]

                img_snippet_resized = cv2.resize(img_snippet, dsize=target_img_size, interpolation=cv2.INTER_CUBIC)
                all_image_snippets[i, :, :, :] = img_snippet_resized
